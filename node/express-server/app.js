...
app.use(function (err, req, res, next) {
  res
    .status(500)
    .json({ statusCode: res.statusCode, errMessage: err.errMessage }); //제이슨으로 상태 코드(500)와 실제 에러 메세지를 보냄
});
...
const getData = (target, where) => {
  //ex  단건조회의 경우 'posts', req.params.id
  let data = jsonData[target]; //객체의 정보가 넘어오면    //[target]은 키값 [post] [comments] [profile]  jsonData[key] = value가 찍히니까
  if (Array.isArray(data)) {
    //data가  Array라면 true, 아니라면 false.  //profile은 배열이 아니니까  배열만 돌릴라고 typeof는 배열을 obj로 봄 안됨  배열/obj 구분이 안됨
    let list = data;
    for (let obj of list) {
      //list배열에있는 obj객체를 하나씩 돌면서
      if (obj.id == where) {
        //select 문이라고 생각 ..?   //where이 없닷면 if문 동작 안하고  원래 list=data 가 동작해서 전체가 나오게 됨
        data = obj;
      }
    }
  }
...
app
  .route("/emps")
  .get((req, res) => {
    //http://localhost:3000/emps 하면 사원전체조회 실행됨 default get이라서
    res.send("사원전체조회");
  })
  .post((req, res) => {
    res.send("사원등록");
