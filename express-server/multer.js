...
const storage = multer.diskStorage({    //디스크 저장소에 대한 객체를 생성  //파일이 저장될 위치 , 파일 명에 대한 것을 정의 
    destination : function(req, file , cb){    
        cb(null, 'uploads/');   //express server내에  uploads폴더가 있어야함 //cd콜백함수를 통해 전송된 파일 저장 디렉터리설정 
    },
    filename : function(req, file, cb){     //사용자가 입력한 파일이름은 중복 가능성이 있기 때문에 유니크한 값을 주기 위해 
...
app.post('/profile' , upload.single('avatar'), (req, res)=>{  //단일 처리    //avatar 어떤 이름으로 받을지 정해줘야함 
...
app.post('/photos', upload.array('photos',12), (req, res)=>{   //'photos',12 이름과 최대 갯수를 제한      <input type="file" name="photos" multiple> name이 key값 ??
    for(let file of req.files){      //여러건 s
        console.log(file);
    }
})
